...
namespace UI
{
    class HeadedPanel: BasePanel
    {
        ...
        protected virtual void SetupTopButtons()
        {
            ...
            TopBar.GetComponent<HorizontalLayoutGroup>().spacing = (Width - totalButtonWidth) / (_topButtons.Count + 1);
        }

        protected override float GetPanelHeight()
        {
            float topBarHeight = TopBar.GetComponent<RectTransform>().sizeDelta.y;
            float bottomBarHeight = BottomBar.GetComponent<RectTransform>().sizeDelta.y;
            return Height - topBarHeight - bottomBarHeight - BorderVerticalPadding * 2f;
        }
    }
}
