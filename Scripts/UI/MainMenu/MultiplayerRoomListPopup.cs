...
namespace UI
{
    class MultiplayerRoomListPopup: BasePopup
    {
        ...
        protected override void SetupPopups()
        {
            ...
            _multiplayerCreatePopup = ElementFactory.CreateHeadedPanel<MultiplayerCreatePopup>(transform).GetComponent<MultiplayerCreatePopup>();
        ...
        public void RefreshList(bool refetch = true)
        {
            ...
            foreach (RoomInfo room in pageRooms)
            {
                ...
                button.GetComponent<Button>().onClick.AddListener(() => OnRoomClick(room.name));
        ...
        protected List<RoomInfo> GetFilteredRooms()
        {
            ...
            foreach (RoomInfo room in _rooms)
            {
                if (!IsValidRoom(room))
                    continue;
                if (_filterQuery.Value != string.Empty && !room.name.ToLower().Contains(_filterQuery.Value.ToLower()))
                    continue;
                if (!_filterShowFull.Value && room.playerCount >= room.maxPlayers)
                    continue;
                if (!_filterShowPassword.Value && GetRoomPassword(room.name) != string.Empty)
                    continue;
        ...
        {
            string[] strArray = info.name.Split(_roomSeperator);
            return strArray.Length > 5;
        }

        protected string GetRoomPassword(string name)
        {
            string[] strArray = name.Split(_roomSeperator);
        ...
        protected string GetRoomFormattedName(RoomInfo room)
        {
            ...
            object[] objArray1 = new object[] { strArray[0], " / ", strArray[1], " / ", strArray[2].UpperFirstLetter(), " / ", strArray[4].UpperFirstLetter(), "   ", room.playerCount, "/", room.maxPlayers };
            return string.Concat(objArray1).hexColor();
        }

        private void OnRoomClick(string name)
        {
            string password = GetRoomPassword(name);
            if (password != string.Empty)
            {
                HideAllPopups();
                _multiplayerPasswordPopup.Show(password, name);
            }
            else
            {
