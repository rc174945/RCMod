...
public class TITAN : Photon.MonoBehaviour
{
    ...
    public bool WillDIe(int damage)
    {
        if (!hasDie)
        {
            ...
            {
                return currentHealth - damage <= 0f;
            }
    ...
    IEnumerator HandleSpawnCollisionCoroutine(float time, float maxSpeed)
    {
        while (time > 0f)
        {
    ...
    public void cache()
    {
        ...
        obj2.layer = PhysicsLayer.PlayerAttackBox;
    ...
    public void explode()
    {
        if (((SettingsManager.LegacyGameSettings.TitanExplodeEnabled.Value) && this.hasDie) && ((this.dieTime >= 1f) && !this.hasExplode))
        {
            ...
            if (num == 1)
            {
                ...
                foreach (GameObject obj2 in GameObject.FindGameObjectsWithTag("Player"))
                {
                    if (Vector3.Distance(obj2.transform.position, position) < SettingsManager.LegacyGameSettings.TitanExplodeRadius.Value)
                    {
    ...
    private void findNearestFacingHero2()
    {
        ...
        {
            float num5 = Vector3.Distance(hero.transform.position, position);
            if (num5 < positiveInfinity)
            {
                Vector3 vector2 = hero.transform.position - this.baseTransform.position;
                ...
                if (Mathf.Abs(f) < num3)
                {
                    obj2 = hero.gameObject;
                    positiveInfinity = num5;
                }
            }
        }
        foreach (TITAN_EREN hero in this.MultiplayerManager.getErens())
        {
            float num5 = Vector3.Distance(hero.transform.position, position);
            if (num5 < positiveInfinity)
            {
    ...
    {
        if ((!GameMenu.Paused || (IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.SINGLE)) && ((IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE) || base.photonView.isMine))
        {
    ...
    public void loadskin()
    {
        ...
        if (((IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE) || base.photonView.isMine) && ((settings.SkinsEnabled.Value)))
        {
            int index = (int) UnityEngine.Random.Range(0f, 5f);
            int num2 = index;
            TitanCustomSkinSet set = (TitanCustomSkinSet)settings.GetSelectedSet();
            if (set.RandomizedPairs.Value)
            {
                num2 = (int)UnityEngine.Random.Range(0f, 5f);
            }
            string body = ((StringSetting)set.Bodies.GetItemAt(index)).Value;
            string eye = ((StringSetting)set.Eyes.GetItemAt(num2)).Value;
            this.skin = index;
    ...
    public IEnumerator loadskinE(string body, string eye)
    {
        while (!this.hasSpawn)
            yield return null;
        yield return StartCoroutine(_customSkinLoader.LoadSkinsFromRPC(new object[] { false, body, eye }));
    }
    ...
    private void Start()
    {
        ...
        if ((this.maxHealth == 0) && (SettingsManager.LegacyGameSettings.TitanHealthMode.Value > 0))
        {
            ...
            {
                this.maxHealth = this.currentHealth = UnityEngine.Random.Range(SettingsManager.LegacyGameSettings.TitanHealthMin.Value, 
                    SettingsManager.LegacyGameSettings.TitanHealthMax.Value + 1);
            }
            else if (SettingsManager.LegacyGameSettings.TitanHealthMode.Value == 2)
            {
                this.maxHealth = this.currentHealth = Mathf.Clamp(Mathf.RoundToInt((this.myLevel / 4f) * UnityEngine.Random.Range(SettingsManager.LegacyGameSettings.TitanHealthMin.Value, 
                    SettingsManager.LegacyGameSettings.TitanHealthMax.Value + 1)), SettingsManager.LegacyGameSettings.TitanHealthMin.Value, SettingsManager.LegacyGameSettings.TitanHealthMax.Value);
            }
        }
    ...
    public void UpdateHeroDistance()
    {
        if ((!GameMenu.Paused || IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.MULTIPLAYER) && myDifficulty >= 0 && !nonAI)
        {
            ...
            else
            {
                ...
                myDistance = Vector2.Distance(heroPosition, basePosition);
            }
        }
    }
    ...
    {
        UpdateHeroDistance();
        if (((!GameMenu.Paused || (IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.SINGLE)) && (this.myDifficulty >= 0)) && ((IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE) || base.photonView.isMine))
        {
            this.explode();
            if (!this.nonAI)
            {
